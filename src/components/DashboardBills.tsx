...
export const DashboardBills: React.FC<DashboardBillsProps> = ({ 
  ...
  const [items, setItems] = useState<BillItem[]>([
    { description: "", quantity: 1, unitPrice: 0, totalPrice: 0, ivaRate: "22%" }
  ]);
  ...
  const handleAddItemRow = () => {
    setItems(prev => [...prev, { description: "", quantity: 1, unitPrice: 0, totalPrice: 0, ivaRate: "22%" }]);
  };
  ...
  const handleSubmitBill = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setManualTotal(0);
        setItems([{ description: "", quantity: 1, unitPrice: 0, totalPrice: 0, ivaRate: "22%" }]);
        setEntryMode("totals");
      } else {
  ...
  const filteredBills = useMemo(() => {
    return billsList.filter(bill => {
      ...
      const matchSearch = 
        ...
        (bill.providerRut || "").toLowerCase().includes(searchLower) ||
        (bill.documentNumber || "").toLowerCase().includes(searchLower) ||
        (bill.notes || "").toLowerCase().includes(searchLower) ||
        (bill.items || []).some(it => it.description.toLowerCase().includes(searchLower));
              ...
              {entryMode === "detailed" ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] uppercase font-extrabold tracking-wider text-zinc-400">Detalle de Artículos de la Boleta</span>
                    <button
                      type="button"
                  ...
                  <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                    {items.map((it, idx) => (
                      <div 
                        key={idx}
                        className="grid grid-cols-1 md:grid-cols-12 gap-2 bg-zinc-900 p-3 rounded-lg border border-zinc-850 items-end"
                      >
                        {/* Description */}
                        <div className="md:col-span-5 space-y-1">
                          <label className="text-[9px] text-zinc-500 font-bold uppercase">Detalle/Artículo</label>
                          <input
                            type="text"
                            ...
                            required
                            className="w-full bg-zinc-950 border border-zinc-800 text-zinc-200 text-xs rounded px-2.5 py-1.5 focus:outline-none focus:border-indigo-500"
                          />
                        </div>

                        {/* Quantity */}
                        <div className="md:col-span-2 space-y-1">
                          <label className="text-[9px] text-zinc-500 font-bold uppercase">Cant.</label>
                          <input
                            type="number"
                            min="1"
                            value={it.quantity}
                            ...
                            required
                            className="w-full bg-zinc-950 border border-zinc-800 text-zinc-200 text-xs rounded px-2.5 py-1.5 focus:outline-none focus:border-indigo-500 text-center font-mono"
                          />
                        </div>

                        {/* Unit Price (with IVA) */}
                        <div className="md:col-span-2 space-y-1">
                          <label className="text-[9px] text-zinc-500 font-bold uppercase">P. Unitario IVA incl.</label>
                          <input
                            type="number"
                            step="any"
                            min="0"
                            ...
                            required
                            className="w-full bg-zinc-950 border border-zinc-800 text-zinc-200 text-xs rounded px-2.5 py-1.5 focus:outline-none focus:border-indigo-500 text-right font-mono"
                          />
                        </div>

                        {/* IVA rate */}
                        <div className="md:col-span-2 space-y-1">
                          <label className="text-[9px] text-zinc-500 font-bold uppercase">Tasa IVA</label>
                          <select
                            value={it.ivaRate}
                            onChange={(e) => handleUpdateItemRow(idx, "ivaRate", e.target.value)}
                            className="w-full bg-zinc-950 border border-zinc-800 text-zinc-300 text-xs rounded px-2 py-1.5 focus:outline-none focus:border-indigo-500"
                          >
